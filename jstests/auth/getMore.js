...
(function() {
    ...
    function runTest(conn) {
        ...
        if (!isMongos) {
            assert.eq(1, testDB.auth("Alice", "pwd"));
            res = assert.commandWorked(
                testDB.runCommand({parallelCollectionScan: "foo", numCursors: 1, batchSize: 0}));
        ...
        res = assert.commandWorked(testDB.runCommand(
            {aggregate: "foo", pipeline: [{$out: "out"}], cursor: {batchSize: 0}}));
        cursorId = res.cursor.id;
        testDB.logout();
        assert.eq(1, adminDB.auth("admin", "admin"));
        ...
        assert.eq(1, testDB.auth("Alice", "pwd"));
        assert.commandFailedWithCode(
            testDB.runCommand({aggregate: "foo", pipeline: [{$out: "out"}]}),
            ErrorCodes.Unauthorized,
        ...
        res = assert.commandWorked(testDB.runCommand({
            aggregate: "foo",
            pipeline: [{$lookup: {from: "bar", localField: "_id", foreignField: "_id", as: "bar"}}],
            cursor: {batchSize: 0}
        }));
        cursorId = res.cursor.id;
        adminDB.logout();
