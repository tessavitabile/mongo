    ...
    while (i.more()) {
        ...
        if (e.fieldName()[0] == '$') {
            ...
            if (mongoutils::str::equals("or", rest)) {
                ...
                // DBRef field.
                // The collator is null because collection names are compared using binary
                // comparison.
                ...
                root->add(eq.release());
            } else if (mongoutils::str::equals("id", rest)) {
                // DBRef field.
                std::unique_ptr<ComparisonMatchExpression> eq =
                ...
                root->add(eq.release());
            } else if (mongoutils::str::equals("db", rest)) {
                // DBRef field.
                // The collator is null because db names are compared using binary comparison.
                CollatorInterface* collator = nullptr;
                std::unique_ptr<ComparisonMatchExpression> eq =
                    stdx::make_unique<EqualityMatchExpression>(collator);
                Status s = eq->init(e.fieldName(), e);
