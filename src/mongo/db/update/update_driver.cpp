    ...
    if (_root) {
        ...
        for (vector<ModifierInterface*>::iterator it = _mods.begin(); it != _mods.end(); ++it) {
            ...
            for (int i = 0; i < ModifierInterface::ExecInfo::MAX_NUM_FIELDS; i++) {
                ...
                // the index, even though "a.1.c" does not appear to affect the index.
                if (!_affectIndices && !execInfo.noOp && _indexedFields &&
                    _indexedFields->mightBeIndexed(
                        execInfo.indexOfArrayWithNewElement[i]
                            ? execInfo.fieldRef[i]->dottedSubstring(
                                  0, *execInfo.indexOfArrayWithNewElement[i] + 1)
                            : execInfo.fieldRef[i]->dottedField())) {
                    _affectIndices = true;
                    doc->disableInPlaceUpdates();
                }
