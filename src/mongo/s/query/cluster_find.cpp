...
StatusWith<CursorResponse> ClusterFind::runGetMore(OperationContext* opCtx,
    ...
    if (!pinnedCursor.getValue().getAuthenticatedUsers().empty()) {
        bool sameUser = false;
        for (UserNameIterator nameIter =
                 AuthorizationSession::get(opCtx->getClient())->getAuthenticatedUserNamesIter();
             nameIter.more();
             nameIter.next()) {
            if (std::find(pinnedCursor.getValue().getAuthenticatedUsers().begin(),
                          pinnedCursor.getValue().getAuthenticatedUsers().end(),
                          *nameIter) != pinnedCursor.getValue().getAuthenticatedUsers().end()) {
                sameUser = true;
                break;
            }
        }
